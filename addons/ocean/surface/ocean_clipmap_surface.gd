...
const BREAKERS_COASTAL_VERTEX := '''
	...
	float pre_lip_core = front_upper_mask * anchored_forward_shape_qualifier;
	float lip_core = tip_mask * anchored_forward_shape_qualifier * clamp(breaker_lip_strength, 0.0, 1.0);
	float lip_front_support = anchored_forward_shape_qualifier * max(front_upper_mask, tip_mask);
