...
vec3 apply_breaker_deformation(vec3 long_displacement, vec4 field, vec4 warp, vec2 coast_uv, float confidence) {
	...
	vec4 breaker_state = textureLod(breaker_lifecycle, world_uv(warp.xy, params.domains.x), 0.0);
	float active_break = smoothstep(0.05, 0.35, breaker_state.r);
	float breaking_g = clamp(textureLod(breaking_activity_long, world_uv(warp.xy, params.domains.x), 0.0).g, 0.0, 1.0);
	float fft_j = textureLod(displacement_long, world_uv(warp.xy, params.domains.x), 0.0).a;
