...
const BREAKERS_COASTAL_VERTEX := '''
	...
	float pre_lip_forward = wavelength_m * max(breaker_pre_lip_forward_fraction, 0.0) * directional_pre_lip_core * front_upper_mask * lift_envelope * pre_lip_activation * breaker_amplitude * phase_seam_guard;
	...
	float pre_lip_lift_raw = positive_crest_height * max(breaker_pre_lip_lift_scale, 0.0) * directional_pre_lip_core * front_upper_mask * lift_envelope * pre_lip_activation * breaker_amplitude * phase_seam_guard;
