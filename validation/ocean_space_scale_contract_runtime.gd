...
func _validate_stable_contract(open_ocean: Node) -> bool:
	...
	if not sources.has("domains") or not spindrift_sources.has("domains") or sources.domains != spindrift_sources.domains:
	...
	var expected_domains := base_domains * expected_horizontal
