...
Ocean Production es el sistema de océano de producción para Godot. Requiere Godot **4.7.1** y entrega mar abierto profundo P0, Coastal Runtime P1, Crest Foam V3 Core P2, Surface Foam V3 P3 cerrado y Water Optics / Refraction V3 P4 listo para revisión visual.
...
Disponible ahora: Open Ocean P0, Coastal Runtime P1, Crest Foam V3 Core P2 y Surface Foam V3 P3 cerrados; P4 Water Optics / Refraction V3 tiene Structural y Runtime PASS, con revisión visual pendiente de Eric. P3 usa un solver auxiliar J-only independiente, Direct-J topology, field persistente, MID history y Crest Filigree. P4 usa una variante de material: con `Optics` OFF no declara ni muestrea screen/depth; con `Optics` ON consume screen/depth y, si existe, únicamente el seabed bake real.
