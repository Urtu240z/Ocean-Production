...
const BREAKERS_COASTAL_VERTEX := '''
	...
	float forward_shape_qualifier = clamp(max(front_face_gate, crest_gate * 0.35), 0.0, 1.0);
	float pre_lip_core = front_upper_mask * forward_shape_qualifier;
	...
	float lip_core = tip_mask * forward_shape_qualifier * clamp(breaker_lip_strength, 0.0, 1.0);
