...
| `Crest Foam Profile` | `OceanCrestFoamProfile` / vacío | Authoring/presentación P2 por LONG/MID/SHORT, breakup, fade y PBR. Vacío usa exactamente los defaults Production aprobados; cambia en caliente sin reconstruir la FFT completa. |
| `Surface Foam Profile` | `OceanSurfaceFoamProfile` / vacío | Authoring/presentación P3 de fuente, lifetime, evolución, MID fold, shaping, fade, PBR y deperiodización activa. Vacío usa exactamente los defaults Production aprobados. |
...
`OceanCrestFoamProfile` contiene además intensity `0.96`, contrast `1.19`, detail `0.35`, breakup `0.45` / `14 m` / edge `0.32`, fade `0–5000 m`, color residual `(1.84945, 1.84945, 1.84945)`, roughness `0.88` y specular `0.18`. Sus cambios actualizan el solver o los uniforms existentes sin rebuild de H0, FFT base, snapshots ni acumuladores.
...
`OceanSurfaceFoamProfile` contiene `whitecap_threshold = 0.58`, `source_gain = 2.05`, `selectivity = 0.28`, `attack = 0.16 s`, `lifetime = 1.10 s`, `evolution_speed = 0.59`, intensity `2.52`, threshold visual `0.11`, fade `200–600 m`, color `(0.8954583, 0.9249786, 0.9149783)`, roughness `0.82`, specular `0.37`, acoplamiento `0.35`, MID fold `0–1` con influencia `1`, deperiodización activa/celda `32 m` y los shaping activos de Crest Filigree. Fuente/tiempo escriben los buffers existentes; presentación escribe uniforms; ninguna de las dos rutas recrea el solver.
...
`OceanReflectionProfile` controla únicamente el aspecto y la estabilidad del
path P5 promovido. La resolución SSPR es `0.40`; temporal está activo con
weight `0.12` y threshold de profundidad `0.035`. Kawase y Near SSR no son
parte de Production.
...
Resoluciones, formatos RD, conteos de buffers, dispatches, ping-pong, topología IFFT, pass budget y scheduler son `INTERNAL`. Si una resolución pasa a depender de calidad, será responsabilidad futura de `OceanQualityProfile`, no de `OceanSurfaceFoamProfile`.
# Reflections / SSPR
